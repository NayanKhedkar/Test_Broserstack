...
(function ($) {
	...
	AblePlayer.prototype.getAvailablePreferences = function() {
		...
		prefs.push({
			...
			'label': null,
			'group': 'transcript',
			'default': PREFERENCES.prefTranscript //0 // off because turning it on has a certain WOW factor
		});
		prefs.push({
		...
		prefs.push({
			...
			'label': null,
			'group': 'transcript',
            'default': PREFERENCES.prefAutoScrollTranscript // 1
		});
		prefs.push({
		...
		if (this.mediaType === 'video') { 
			...
			prefs.push({
				...
				'group': 'descriptions',
				'default': PREFERENCES.prefDescRate
			});
			prefs.push({
				'name': 'prefDescVolume',
			...
			prefs.push({
				...
				'group': 'descriptions',
                'default': PREFERENCES.prefDescVisible || 0 // off as of 4.3.16, to avoid overloading the player with visible features
			});
		}
...
(function ($) {
	...
	AblePlayer.prototype.calculateControlLayout = function () {
		...
		if ((this.transcriptType === 'popup'||this.transcriptType === 'external') && !(this.hideTranscriptButton)) {
			numA11yButtons++;
			if (this.skin === 'legacy') {
		...
		if (this.browserSupportsVolume()) {
			...
			// extra code added mute/unmute functionality
			controlLayout[1].push('volume');
...
(function ($) {
	...
	window. AccessibleSlider = function(mediaType, div, orientation, length, min, max, bigInterval, label, className, trackingMedia, initialState) {
		...
		this.bodyDiv.on(
			...
			coords = thisObj.pointerEventToXY(e);
            // extra code added
            if (thisObj.seekHead.length && IS_DISABLE_SEEK_BAR) return;
...
(function ($) {
	...
	AblePlayer.prototype.setPlaybackRate = function (rate) {
		// extra code added
        rate = rate <= MAX_PlAYBACK_RATE ? rate : MAX_PlAYBACK_RATE;
	...
	AblePlayer.prototype.playMedia = function () {
        ...
        onPlay(thisObj); // extra code added
	};
		...
		if (context === 'playpause' || context == 'init'){
			if (typeof this.$bigPlayButton !== 'undefined' && typeof this.seekBar !== 'undefined') {
				...
				if (this.paused && !this.seekBar.tracking) {
					...
					if (this.fullscreen) {
						// extra code added
						var transcriptWidth = (typeof this.$transcriptArea !== 'undefined') && this.$transcriptArea.is(':visible') ? this.$transcriptArea.width() : 0;
                        this.$bigPlayButton.width($(window).width() - transcriptWidth);
		...
		if (captions.length === 1) {
			...
			if (this.captionsOn === true) {
				...
				if (this.usingYouTubeCaptions) {
                    ...
                    onHideCaption(this); //extra code added
				}
			}
			else {
				...
				if (this.usingYouTubeCaptions) {
                    ...
                    onShowCaption(this); //extra code added
				}
	...
	AblePlayer.prototype.handleDescriptionToggle = function() {
		...
		if (typeof this.$descDiv !== 'undefined') {
			if (!this.$descDiv.is(':hidden')) {
				this.$descDiv.hide();
                onHideDescription(this);//extra code added
			}
	...
	AblePlayer.prototype.handleTranscriptToggle = function () {
		...
		if (this.$transcriptDiv.is(':visible')) {
            ...
            onHideTranscript(this); //extra code added
            ...
            onShowTranscript(this); //extra code added 
	...
	AblePlayer.prototype.setFullscreen = function (fullscreen) {
		...
		var el = $el.closest('.player-wrapper').length ? $el.closest('.player-wrapper')[0] : $el[0];
		...
		$(document).on('webkitfullscreenchange mozfullscreenchange fullscreenchange MSFullscreenChange', function(e) {
			...
			thisObj.resizePlayer();
			//extra code added
		    handleFullscreenToggle(thisObj);
	...
	AblePlayer.prototype.resizePlayer = function (width, height) {
		...
		if (typeof width !== 'undefined' && typeof height !== 'undefined') { 
			...
			newWidth = $(window).width() - transcriptWidth;			
			newHeight = $(window).height() - this.$playerDiv.outerHeight() - 5 - (this.descOn ? this.$descDiv.height() : 0);
		...
		if (this.player === 'youtube' || this.player === 'vimeo') { 
			...
			if (this.fullscreen) {
				// custom code to get transcript width for fullscreen and update player width
				var transcriptWidth = (typeof this.$transcriptArea !== 'undefined') && this.$transcriptArea.is(':visible') ? this.$transcriptArea.width() : 0;
				//if (typeof this.$captionsWrapper!== 'undefined') {
					// this.$ableWrapper.css({
					// 	'width': newWidth - transcriptWidth + 'px',
					// 	'height': newHeight
					// });
					// this.$captionsWrapper.css({
					// 	'height': (newHeight - this.$playerDiv.height() - (this.descOn ? this.$descDiv.height() : 0)) + 'px',
					// 	'width': newWidth - transcriptWidth
					// });
					// this.$media.css({
					// 	'height': (newHeight - this.$playerDiv.height() - (this.descOn ? this.$descDiv.height() : 0)) + 'px',
					// 	'width': newWidth - transcriptWidth
	...
	AblePlayer.prototype.updateZIndex = function(which) {
		...
		if (transcriptZ === signZ) {
			...
			if (which =updateTranscript== 'transcript') {
...
(function ($) {
	...
	AblePlayer.prototype.injectTranscriptArea = function() {
		...
		if (!this.prefTranscript && !this.transcriptDivLocation) {
			this.$transcriptArea.hide();
            onHideTranscript(this); //extra code added
			//extra code added to handle player size on transcript hide and show
            if (this.isFullscreen) {
                //thisObj.onWindowResize();
            }
		}
...
(function ($) {
	...
	AblePlayer.prototype.onMediaComplete = function () {
		...
		mediaCompleted(); // extra code added
	...
	AblePlayer.prototype.onClickPlayerButton = function (el) {
        ...
        // extra code added to disable player event temporary
        if (IS_PLAYER_DISABLE) return;
		...
		// extra code addded for xAPI
        // if (window.parent.sendStatement != undefined) {
        // 	window.parent.sendStatement("clicked", "http://custom.verb.com/clicked", whichButton + " button");
        // }
	};
